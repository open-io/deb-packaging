...
repo_name="puppet-diamond"
src=$repo_name
pkg=puppet-module-openio-diamond
release=1
...
pushd ${src}-${id}
dch --force-distribution -b -v ${cdate}.git${id:0:7}-${release} --distribution jessie 'Testing'
dpkg-buildpackage -S -us -uc -nc
popd
echo "sudo DIST=jessie pbuilder build ${pkg}_${cdate}.git${id:0:7}-${release}.dsc"
